...
int
gpdata_get_atomic_number_for_symbol (const char *sym)
{
  int i;
...
void
gpparse_print_element_data_for_num (int num)
{
  int row, n, i;
  ...
  for (row = 0; row < MAX_INFO_NR; row++)
    {
      ...
      for (i = 0; i < HEADERTEXTLEN; i++)
        fillout[i] = ' ';
      fillout[n] = (char) NULL;
      printf (" %s%s%s\n", header.info[row], fillout,
              table[num - 1].info[row]);
    }
  printf ("\n");
}
...
void
main_window_exit (GtkWidget * w, gpointer d)
{
  gtk_main_quit ();
}
...
void
dialog_destroy (GtkWidget * w, gpointer d)
{
...
void
dialog_close (GtkWidget * w, gpointer d)
{
...
void
display_element_dialog (GtkWidget * w, gpointer d)
{
  ...
  _DBG (g_message
        ("%s(%d) display_table_element() w=0x%08x d=0x%08x", __FILE__,
         __LINE__, w, d));
...
int
criterion2scale (double r, int maxcolor)
{
  int result;
...
main_prog (void)
{
  ...
  GtkActionEntry entries[] = {
    ...
    {"Exit", GTK_STOCK_OPEN, _("E_xit"), "<control>Q", _("Exit the program"),
     (GCallback) menu_file_exit},
    {"About", GTK_STOCK_HELP, _("_About"), "<control>H", _("About gperiodic"),
     (GCallback) menu_help_about},
  };
...
/* connect the close event... */
  g_signal_connect (GTK_OBJECT (window), "delete-event",
                    G_CALLBACK (main_window_exit), &window);
...
/******************************************************************************/
      {
        ...
        _DBG (g_message
              (N_("%s(%d) Setting color:  %d %d %d"), __FILE__, __LINE__,
               table[i].color.red, table[i].color.green,
               table[i].color.blue));
        ...
        if (enable_colorscale)
          {
            colorize_element (i,
                              ...
                              &condensed_gas_cs, condensed_gas_crit, 0, 0);
          }
        else
          {
            /* set for 0, 1, and 2 */
...
/* attach the button to the table */
      gtk_table_attach (GTK_TABLE (periodic_table), table[i].button,
                        table[i].x - 1, table[i].x, table[i].y - 1,
                        table[i].y, GTK_FILL, GTK_FILL, 2, 2);
      ...
      switch (ch)
        ...
        case 'c':
          ...
          if (ambientTemperature < 0)
            {
              printf (N_
                      (" Option -c error : %s is not a valid number (must be > 0).\n"),
                      optarg);
              return -1;
            }
          enable_colorscale = 1;
      ...
      while (optind < argc)
        {
          ...
          if (!atomicNumber)
            {
              atomicNumber =
                gpdata_get_atomic_number_for_symbol (argv[optind]);
            }
          if (atomicNumber == -1)
            {
